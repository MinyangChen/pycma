...
class GPSHADEOptimizer(SHADEOptimizer):
    """
    SHADE with GP surrogate ranking and top-K true evaluations per generation.

    Per generation:
    1) ask() creates trial candidates
    2) surrogate ranks candidates (LCB or posterior mean)
    3) top-K ranked trials are truly evaluated
    4) parent-based SHADE selection/replacement is applied on evaluated trials only
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.top_k_true = int(np.clip(int(top_k_true), 1, self.pop_size))
    ...
    def tell_one(
        ...
        """
        Apply one parent-based SHADE replacement.

        Returns (improved, F, CR, delta), where delta is improvement magnitude.
        """
        ...
        trial = np.clip(np.asarray(trial, dtype=float), lower, upper)
        trial_fit = float(trial_fit)
        parent_fit = float(st.fitness[i_parent])

        if trial_fit < parent_fit:
            replaced_parent = st.population[i_parent].copy()
            st.population[i_parent] = trial
            st.fitness[i_parent] = trial_fit
            st.archive = np.vstack((st.archive, replaced_parent[None, :]))

            if trial_fit < st.best_f:
                st.best_f = trial_fit
                st.best_x = trial.copy()

            return True, float(Fi), float(CRi), parent_fit - trial_fit

        return False, float(Fi), float(CRi), 0.0

    def tell_many(
        self,
        trials: Sequence[np.ndarray],
        trial_fitness: Sequence[float],
        info: Sequence[Tuple[int, float, float]],
        lower: np.ndarray,
        upper: np.ndarray,
    ) -> None:
        """
        Apply parent-based replacement for all evaluated trials in one generation.
        """
        if self.state is None:
            raise RuntimeError("Optimizer state not initialized. Call optimize first.")
        st = self.state
        ...
        for trial, fit, one_info in zip(trials, trial_fitness, info):
            improved, Fi, CRi, delta = self.tell_one(trial, fit, one_info, lower, upper)
            if improved:
                success_F.append(Fi)
        ...
        # One generation increment per ask/rank/evaluate cycle.
        st.generation += 1
    ...
    def optimize(self, problem: Any, max_evals: int) -> Tuple[np.ndarray, float, List[float]]:
        ...
        while st.evals < max_evals:
            ...
            remaining_budget = max_evals - st.evals
            k_eval = min(self.top_k_true, int(pred_order.size), int(remaining_budget))
            if k_eval <= 0:
                break

            selected_idx = np.asarray(pred_order[:k_eval], dtype=int)
            selected_trials = trials_clipped[selected_idx]
            selected_info = [info[int(i)] for i in selected_idx]

            selected_fits = np.asarray(eval_batch(selected_trials), dtype=float).reshape(-1)
            st.evals += int(selected_fits.size)

            self.tell_many(selected_trials, selected_fits, selected_info, lower, upper)
            archive.add_batch(selected_trials, selected_fits)
            history.append(st.best_f)

            if self.print_every > 0 and (st.generation % self.print_every == 0):
                print(
                    f"[GP-SHADE] iter {st.generation:4d} | true_evals {st.evals:6d} | best f = {st.best_f:.6e}"
                )
...
GP_SHADEOptimizer = GPSHADEOptimizer
