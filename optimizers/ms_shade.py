...
class MSSHADEOptimizer(SHADEOptimizer):
    """Mimic Surrogate SHADE: evaluates only one trial per generation chosen by a mimic surrogate ranker."""
    ...
    def tell_one(
        ...
        """
        Update SHADE state using a single evaluated trial.
        ...
        # Update historical memories if there was a successful parameter
        if success_F:
    ...
    def optimize(self, problem: Any, max_evals: int) -> Tuple[np.ndarray, float, List[float]]:
        """Run MS-SHADE using one true evaluation per generation guided by the mimic surrogate."""
        if max_evals <= 0:
