...
class GriewankProblem:
    """Griewank benchmark optimisation problem with a stored random optimum."""
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        """
        Vectorized Griewank function shifted to a stored optimum.
        """
        x = np.atleast_2d(x)
        shifted = x - self._optimum
        sum_term = np.sum(shifted**2, axis=1) / 4000.0
        indices = np.arange(1, self.dim + 1)
        prod_term = np.prod(np.cos(shifted / np.sqrt(indices)), axis=1)
