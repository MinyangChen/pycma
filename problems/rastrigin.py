...
class RastriginProblem:
    """Rastrigin benchmark optimisation problem with a stored random optimum."""
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        """
        Vectorized Rastrigin function shifted to a stored optimum.
        """
        x = np.atleast_2d(x)
        shifted = x - self._optimum
        return 10.0 * self._dim + np.sum(shifted**2 - 10.0 * np.cos(2.0 * np.pi * shifted), axis=1)
