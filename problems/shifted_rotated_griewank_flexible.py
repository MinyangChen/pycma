...
class ShiftedRotatedGriewankFlexible:
    """Griewank function with shift/rotation for arbitrary dimensions."""
    ...
    def __init__(
        ...
        self._name = name or f"shifted_rotated_griewank_dim{dim}"
        self._rng = rng or np.random.default_rng()

        base_dir = os.path.dirname(os.path.abspath(__file__))
        if optimum_path is None:
            optimum_path = os.path.join(base_dir, f"griewank_optimum_dim{dim}.txt")
        self._optimum_path = optimum_path

        if rotation_path is None:
            rotation_path = os.path.join(base_dir, f"shifted_rotated_griewank_rotation_dim{dim}.txt")
