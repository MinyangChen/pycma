...
from problems.slightly_rotated_ellipsoid import SlightlyRotatedEllipsoidProblem


class HierarchySubsystemProblem6:
    """Hierarchy benchmark variant with Ackley blocks replacing the Schwefel layer and an easier ellipsoid top."""

    def __init__(self, name: str = "hierarchy_subsystem_6") -> None:
        self._dim = 100
        self._lower_bound = -32.768
        self._upper_bound = 32.768
        self._name = name

        self._griewank_block_first = ShiftedRotatedGriewankProblem(dim=10)
        ...
        self._top_ellipsoid = SlightlyRotatedEllipsoidProblem(dim=10)
    ...
    def evaluate_with_details(self, x: np.ndarray):
        ...
        z_vals.append(block_value(self._scale_block(chunks[0], self._griewank_block_first), self._griewank_block_first, "ShiftedRotatedGriewank_1", 0))
        z_vals.append(block_value(self._scale_block(chunks[1], self._griewank_block_first), self._griewank_block_first, "ShiftedRotatedGriewank_2", 1))
        z_vals.append(block_value(self._scale_block(chunks[2], self._griewank_block), self._griewank_block, "ShiftedRotatedGriewank_3_Block", 2))
        z_vals.append(block_value(self._scale_block(chunks[3], self._griewank_block), self._griewank_block, "ShiftedRotatedGriewank_4_Block", 3))
        ...
        z_vals.append(block_value(self._scale_block(chunks[6], self._ackley_block), self._ackley_block, "ShiftedRotatedAckley_1", 6))
        z_vals.append(block_value(self._scale_block(chunks[7], self._ackley_block), self._ackley_block, "ShiftedRotatedAckley_2", 7))
        ...
        # def block_value_10(block, problem, label, idx):
        #     vals = problem(block)
        #     detail[label] = vals
        #     return np.log10(vals + 1.0) + top_opt[idx]
        ...
        # z_vals.append(block_value(self._scale_block(chunks[6], self._ackley_block), self._ackley_block, "ShiftedRotatedAckley_1", 6))
        # z_vals.append(block_value(self._scale_block(chunks[7], self._ackley_block), self._ackley_block, "ShiftedRotatedAckley_2", 7))
        # z_vals.append(block_value_10(self._scale_block(chunks[8], self._ellipsoid_block), self._ellipsoid_block, "ShiftedRotatedEllipsoid_1", 8))
        # z_vals.append(block_value_10(self._scale_block(chunks[9], self._ellipsoid_block), self._ellipsoid_block, "ShiftedRotatedEllipsoid_2", 9))

        z = np.stack(z_vals, axis=1)
        top_value = self._top_ellipsoid(z)
        detail["Top_SlightlyRotatedEllipsoid"] = top_value
