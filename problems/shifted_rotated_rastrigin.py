...
class ShiftedRotatedRastriginProblem:
    """Rastrigin function with fixed-dimensional shift and rotation."""
    ...
    def __init__(
        ...
        name: str = "shifted_rotated_rastrigin",
        ...
        if dim != 10:
            raise ValueError("ShiftedRotatedRastriginProblem is defined for dim=10.")
        ...
        if optimum_path is None:
            optimum_path = os.path.join(base_dir, "rastrigin_optimum.txt")
        self._optimum_path = optimum_path

        if rotation_path is None:
            rotation_path = os.path.join(base_dir, "shifted_rotated_rastrigin_rotation.txt")
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        ...
        return 10.0 * self._dim + np.sum(rotated**2 - 10.0 * np.cos(2.0 * np.pi * rotated), axis=1)
