...
class RotatedEllipsoidProblem:
    """Rotated (weighted) ellipsoid optimisation problem with a stored random optimum."""
    ...
    def evaluate(self, x: np.ndarray) -> np.ndarray:
        """
        Vectorized rotated ellipsoid (weighted sum of squares) function shifted to a stored optimum.
        """
        x = np.atleast_2d(x)
